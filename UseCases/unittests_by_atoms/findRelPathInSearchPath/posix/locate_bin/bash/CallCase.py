...
class CallUnits(unittest.TestCase):
    ...
    def testCase000_locate_bash(self):
        px = filesysobjects.FileSysObjects.findRelPathInSearchPath('bash',os.environ['PATH'])
        if not px:
            px = filesysobjects.FileSysObjects.findRelPathInSearchPath('bash.exe',os.environ['PATH'])
        if not px:
