...
class CallUnits(unittest.TestCase):
    ...
    def testCase014(self):
        _call  = self._call
        _call += " EXITNOK "
        retX = [0,['fromE', 'arbitrary output', 'arbitrary signalling OK string', 'arbitrary output'],[]]
        ret = self.sx.callit(_call)
        self.assertEqual(ret,retX)
    ...
    def testCase015(self):
        _call  = self._call
        _call += " EXIT7 "
        retX = [0,['fromF', 'arbitrary output', 'arbitrary signalling NOK string', 'arbitrary output'],[]]
        ret = self.sx.callit(_call)
        self.assertEqual(ret,retX)
    ...
    def testCase016(self):
        _call  = self._call
        _call += " EXIT8 "
        retX = [0,['fromG', 'arbitrary output', 'arbitrary signalling NOK string', 'arbitrary output'],['arbitrary err output', 'arbitrary err signalling NOK string', 'arbitrary err output']]
        ret = self.sx.callit(_call)
        self.assertEqual(ret,retX)
